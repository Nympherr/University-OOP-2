...
void vidurkio_vaizdavimas(std::list<studentas> &asmuo){
    ...
    for (const auto& persona : asmuo) {
        std::cout << std::fixed << std::setprecision(2);
        std::cout << std::setw(21) << std::left << persona.pavarde;
        std::cout << std::setw(21) << std::left << persona.vardas;
        std::cout << persona.galutinis_balas << '\n';
    }
...
// jeigu vartotojas nori pridėti naują įrašą
    ...
    do{
        ...
        if(ivestis == "t"){
            ...
            duomenu_suvedimas(asmenys.back());
            mokiniu_dydis++;
        }
    }while(ivestis == "t");
};
...
void failo_irasymas_paprastai(const std::string& failo_pavadinimas, const std::list<studentas>& sarasas) {
...
void failo_studento_nuskaitymas(std::string failo_pavadinimas){
    ...
    std::list<std::string> nuskaitytas_failas;
    ...
    std::list<std::vector<std::string>> rezultatas;
    ...
    for (const auto& result : rezultatas) {
        ...
        for(int j = 0; j < n ; j++){
            ...
            if(j == 0){
                asmenys.back().vardas = result[j]; 
            }
            else if(j == 1){
                asmenys.back().pavarde = result[j];
            }
            else if(j > 1 && j < n - 1){
                try{
                    int reiksme = std::stoi(result[j]);
                    asmenys.back().pazymiai.push_back(reiksme);
                }
                ...
                try{
                int reiksme = std::stoi(result[j]);
                asmenys.back().egzaminas = reiksme;
                }
                catch (const std::invalid_argument& e) {
                    asmenys.back().egzaminas = 0;
                }
            }   
        }
        galutinio_balo_skaiciavimas(asmenys.back());
        medianos_skaiciavimas(asmenys.back());
    }
    ...
    std::cout << "Nuskaityto failo duomenų pertvarkymas(studentų sudėjimas į list'ą):  "<< diff_sutvarkyti_duomenys.count() << " s\n"; 
};
...
void failo_nuskaitymas(){
   ...
   while (true) {
    ...
    asmenys.sort(galutinio_balo_rusiavimas);
    ...
    for (const auto& asmuo : asmenys) {
        if(asmuo.galutinis_balas >= 5.0){
            asmenys_kietakai.push_back(asmuo);
        }
        else{
            asmenys_vargsiukai.push_back(asmuo);
        }
    }

    // ištrina bendrą vektorių, kadangi nebenaudojame

    for (auto& asmuo : asmenys) {
        asmuo.pazymiai.clear();
    }
    asmenys.clear();
